...
class Foveation(torch.nn.Module):
    ...
    def pyramid(self, im, sigma=1, prNum=6):
        ...
        TORCHKERNEL = torch.zeros(3,3,5,5)
        for i in range(3):
            TORCHKERNEL[i,i] = torch.from_numpy(Gaus_kernel2D)

        # downsample
        for i in range(1, prNum):
            t = torch.nn.functional.conv2d(
                    torch.nn.functional.pad(
                        torch.tensor(G).transpose(0,2).unsqueeze(0).float(), 
                        (2,2,2,2),
                        mode='reflect',
                    ),
                    TORCHKERNEL
                ).squeeze().transpose(2,0)
            G = t.numpy()
    ...
    def foveat_img(self, im, fixs):
        ...
        x2d, y2d = torch.meshgrid(torch.arange(width), torch.arange(height), indexing='xy')
        ...
        Ts2 = torch.cat([torch.exp(-((2 ** (torch.arange(1,prNum)-3).reshape(-1,1,1)) * R / sigma) ** 2 * k), torch.zeros_like(theta).unsqueeze(0)])
        breakpoint()
...
if __name__ == '__main__':
    foveat = Foveation(crop_size=180)

    I = torch.randn(1,3,180,180)
    foveat(I)
