...
class SimCLRTrainDataTransform:
    ...
    def __init__(
        self, input_height: int = 224, gaussian_blur: bool = True, jitter_strength: float = 1.0, normalize=None, count=2
    ) -> None:
    ...
    def __call__(self, sample):
        ...
        l = [transform(sample) for _ in range(self.count - 1)]
            
        # xi = transform(sample)
        # xj = transform(sample)

        return [*l, self.online_transform(sample)]
